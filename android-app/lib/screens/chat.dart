...
class _ChatScreenState extends State<ChatScreen>{
 ...
 String _stripEmoji(String value){final b=StringBuffer();for(final r in value.runes){final isEmoji=(r>=0x1F000&&r<=0x1FAFF)||(r>=0x2600&&r<=0x27BF)||(r>=0x2300&&r<=0x23FF)||(r>=0xFE00&&r<=0xFE0F)||(r>=0x200D&&r<=0x200D);if(!isEmoji)b.writeCharCode(r);}return b.toString();}
 String _ttsText(String text){var s=text; s=s.replaceAll(RegExp(r'```[\s\S]*?```'), ' '); s=s.replaceAll(RegExp(r'`([^`]*)`'), r'\1'); s=s.replaceAll(RegExp(r'https?://\S+'), ' '); s=s.replaceAll(RegExp(r'[*_#~>]+'), ' '); s=_stripEmoji(s); s=s.replaceAll(RegExp(r'[\[\]{}<>|\\]+'), ' '); s=s.replaceAll(RegExp(r'\s+'), ' ').trim(); return s;}
 Future<void> _listenForBargeIn(String spokenText)async{
  if(!voiceMode||!mounted||_bargeInListening)return;
  _bargeInListening=true;
  final spoken=_ttsText(spokenText).toLowerCase().replaceAll(RegExp(r'\s+'),' ').trim();
  try{
    final ok=await speech.initialize(onStatus:(_){},onError:(_){ });
    if(!ok||!voiceMode||!mounted)return;
    await speech.listen(listenOptions:stt.SpeechListenOptions(listenFor:const Duration(seconds:60),pauseFor:const Duration(seconds:2),partialResults:true,listenMode:stt.ListenMode.dictation,cancelOnError:false),onResult:(r){
      final words=r.recognizedWords.trim();
      if(words.isEmpty||!voiceMode)return;
      final normalized=words.toLowerCase().replaceAll(RegExp(r'\s+'),' ').trim();
      // Ignore speech recognition that is simply hearing the assistant's own TTS output.
      final likelyEcho=spoken.isNotEmpty&&(spoken.contains(normalized)||normalized.contains(spoken)||spoken.startsWith(normalized));
      if(likelyEcho)return;
      _ttsInterrupted=true;
      _bargeInListening=false;
      unawaited(tts.stop());
      unawaited(speech.stop());
      lastVoiceText=words;
      voiceWords.value=words;
      if(mounted)setState(()=>listening=false);
      Future.microtask(()async{if(voiceMode&&!voiceSending&&!busy)await _submitVoiceWords();});
    });
  }catch(_){}
}
 Future<void> speak(String text,{bool continueVoice=false})async{final clean=_ttsText(text);if(clean.isEmpty)return;voicePhase.value='speaking';_ttsInterrupted=false;await tts.stop();await tts.awaitSpeakCompletion(true);await tts.setLanguage(language=='mr'?'mr-IN':language=='hi'?'hi-IN':'en-IN');await tts.setSpeechRate(.48);final voices=await tts.getVoices;try{
  ...
  if(chosen==null){final wanted=availableVoices.where((v){final n='${v['name']}'.toLowerCase(),l='${v['locale']}'.toLowerCase();return l.startsWith('hi')||l.startsWith('en')?(voiceGender=='female'?RegExp('female|heera|swara|veena|zira|samantha|hindi.*f|x-hia|x-hic').hasMatch(n):RegExp('male|ravi|hemant|david|mark|hindi.*m|x-hid|x-hie').hasMatch(n)):false;}).toList();if(wanted.isNotEmpty)chosen=wanted.first;}
...
if(voiceMode&&mounted)unawaited(_listenForBargeIn(clean));
await tts.speak(clean);
await speech.stop();_bargeInListening=false;
if(continueVoice&&voiceMode&&mounted&&!_ttsInterrupted){await Future.delayed(const Duration(milliseconds:220));if(voiceMode&&mounted){voiceRestarting=true;voicePhase.value='listening';await mic(true,0);voiceRestarting=false;}}else if(voiceMode&&!_ttsInterrupted){voicePhase.value='ready';}
}
 Future<Map<String,dynamic>> _clientContext({bool includeLocation=false})async{
 ...
 bool _needsLocation(String text){return RegExp(r'\b(location|where am i|near me|nearby|directions|map|weather|temperature|address|current place)\b|लोकेशन|स्थान|कहाँ हूँ|पास में|नजदीक|मौसम|तापमान|पता',caseSensitive:false).hasMatch(text);}
 Future<void> send([String? value,bool speakReply=false])async{final text=(value??input.text).trim();if((text.isEmpty&&attachmentData==null)||busy)return;FocusScope.of(context).unfocus();final sentText=text.isEmpty?'Attached file: ${attachmentName??'file'}':text;setState((){messages.add({'role':'user','content':attachmentName==null?sentText:'$sentText\n📎 $attachmentName'});input.clear();busy=true;});await save();try{String spoken='';if(wantsVideo(sentText)){voicePhase.value='thinking';final ctx=await _clientContext(includeLocation:true);String? previousImage;for(final m in messages.reversed){final c=m['content']??'';if(m['role']=='assistant'&&c.startsWith('[[IMAGE]]')){previousImage=c.substring(9);break;}}final job=await api.request('/ai/video',method:'POST',body:{'prompt':sentText,if(previousImage!=null)'imageDataUrl':previousImage,...ctx});final r=await waitJob('${job['id']}',video:true);final uri=(r['videoUri']??r['videoUrl']??'').toString();final answer=uri.isEmpty?'वीडियो तैयार हो गया है।':'[[VIDEO]]$uri';setState(()=>messages.add({'role':'assistant','content':answer}));spoken='वीडियो तैयार हो गया है।';}else if(wantsImage(sentText)){voicePhase.value='thinking';final job=await api.request('/ai/photo',method:'POST',body:{'prompt':sentText});final r=await waitJob('${job['id']}');final img=(r['imageDataUrl']??r['imageUrl']??'').toString();if(img.isEmpty)throw Exception('Image generated but image data missing');setState(()=>messages.add({'role':'assistant','content':'[[IMAGE]]$img'}));spoken='इमेज तैयार हो गई है।';}else{voicePhase.value='thinking';final history=messages.where((m)=>m['role']=='user'||m['role']=='assistant').toList();final ctx=await _clientContext(includeLocation:_needsLocation(sentText));final r=await api.request('/ai/chat',method:'POST',body:{'message':sentText,'history':history.length>20?history.sublist(history.length-20):history,'voiceGender':voiceGender,'language':language,...ctx,if(attachmentData!=null)'attachmentName':attachmentName,if(attachmentData!=null)'attachmentMime':attachmentMime,if(attachmentData!=null)'attachmentData':attachmentData});final answer=(r['answer']??r['description']??'').toString().trim();if(answer.isEmpty)throw Exception('AI returned an empty response');setState(()=>messages.add({'role':'assistant','content':answer}));spoken=answer;}if((speakReply||voiceMode)&&spoken.isNotEmpty)await speak(spoken,continueVoice:false);if(mounted)setState((){attachmentName=null;attachmentMime=null;attachmentData=null;});}catch(e){final raw=e.toString().replaceFirst('Exception: ','');String msg='⚠️ $raw';if(raw.contains('ALL_IMAGE_PROVIDERS_EXHAUSTED'))msg='⚠️ अभी image generation की सभी configured AI services उपलब्ध नहीं हैं। कृपया थोड़ी देर बाद फिर कोशिश करें।';else if(raw.contains('IMAGE_PROVIDER_BILLING_REQUIRED'))msg='⚠️ Image generation service के लिए provider access/billing चाहिए।';else if(raw.contains('ALL_AI_PROVIDERS_EXHAUSTED'))msg='⚠️ अभी सभी configured AI services उपलब्ध नहीं हैं। कृपया थोड़ी देर बाद फिर कोशिश करें।';else if(raw.contains('GEMINI_DAILY_QUOTA')){quotaKind='daily';quotaUntil=DateTime.now().add(const Duration(hours:24));msg='आज की AI उपयोग सीमा पूरी हो गई है। अगले quota reset के बाद फिर कोशिश करें।';}else if(raw.contains('VIDEO_PROVIDER_BILLING_REQUIRED'))msg='⚠️ Video generation ke liye Google billing/model access chahiye.';else if(raw.contains('GEMINI_RATE_LIMIT')||raw.contains('429')){quotaKind='minute';quotaUntil=DateTime.now().add(const Duration(minutes:1));msg='अभी बहुत requests आ गई हैं। थोड़ी देर बाद फिर कोशिश करें।';}if(mounted)setState(()=>messages.add({'role':'assistant','content':msg}));}finally{busy=false;await save();if(mounted)setState((){});Future.delayed(const Duration(milliseconds:40),()=>scroll.hasClients?scroll.animateTo(scroll.position.maxScrollExtent,duration:const Duration(milliseconds:140),curve:Curves.easeOut):null);}}
