...
try{maintenance=await api.request('/system/status');}catch(_){}try{final w=await api.request('/wallet');coins=(w['coinBalance']??0) as int;}catch(_){}if(mounted)setState((){});}
 ...
 Future<Map<String,dynamic>> _clientContext()async{
  ...
  try {
    final d=await const MethodChannel('com.niteshkhobragade.creator_studio/device').invokeMethod<Map<dynamic,dynamic>>('getDeviceInfo');
    if(d!=null){
      out['deviceManufacturer']='${d['manufacturer']??''}';
      out['deviceModel']='${d['model']??''}';
      out['deviceBrand']='${d['brand']??''}';
      out['deviceAndroidVersion']='${d['androidVersion']??''}';
      final b=int.tryParse('${d['batteryPercent']??''}'); if(b!=null&&b>=0) out['batteryPercent']=b;
    }
  } catch(_) {}
  try{
 ...
 Future<void> fresh()async{messages=[];lastVoiceText='';voiceWords.value='';await speech.stop();await tts.stop();attachmentName=null;attachmentMime=null;attachmentData=null;await save();if(mounted)setState((){});}
 ...
 Future<void> openVoiceConversation()async{if(voiceMode)return;voiceMode=true;voicePhase.value='listening';voiceWords.value='';await speech.stop();await tts.stop();await save();
  final ctx=await _clientContext();
  liveVoice=LiveVoiceSession(onPhase:(p){if(mounted)voicePhase.value=p;},onWords:(w){if(mounted)voiceWords.value=w;},onError:(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(e)));});try{await liveVoice!.start(voiceGender,context:{'timeZone':ctx['timeZone'],'localDateTime':ctx['localDateTime'],'locationLabel':ctx['locationName'],'deviceManufacturer':ctx['deviceManufacturer'],'deviceModel':ctx['deviceModel'],'deviceAndroidVersion':ctx['deviceAndroidVersion'],'batteryPercent':ctx['batteryPercent']});}catch(e){voiceMode=false;voicePhase.value='error';if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(e.toString().replaceFirst('Exception: ',''))));return;}if(!mounted)return;await Navigator.of(context).push(MaterialPageRoute<void>(fullscreenDialog:true,builder:(_)=>_KhobragadeVoicePage(phase:voicePhase,words:voiceWords,gender:voiceGender)));if(voiceMode)await stopVoiceConversation();}
