...
class _ChatScreenState extends State<ChatScreen>{
 ...
 Future<void> load()async{final p=await SharedPreferences.getInstance();try{deviceModel=await const MethodChannel('com.niteshkhobragade.creator_studio/device').invokeMethod<String>('getDeviceModel')??'';}catch(_){}final saved=p.getStringList('khobragade_ai_chat')??[];for(final x in saved){final k=x.indexOf('|');if(k>0)messages.add({'role':x.substring(0,k),'content':x.substring(k+1)});}voiceMode=false;await p.setBool('kh_voice_mode',false);voiceGender=p.getString('kh_voice_gender')??'female';voiceName=p.getString('kh_voice_name')??'';try{
...
try{maintenance=await api.request('/system/status');}catch(_){}try{final w=await api.request('/wallet');coins=(w['coinBalance']??0) as int;}catch(_){}if(mounted)setState((){});}
 ...
 String imagePrompt(String t)=>t.replaceAll(RegExp(r'^(demo\s*)?(image|photo|picture|tasveer|tasvir|तस्वीर|इमेज|फोटो|चित्र)\s*(generate|create|bana|banao|banado|जनरेट|क्रिएट|बना|बनाओ|बना दो)?\s*',caseSensitive:false), '').trim().isEmpty?t:t;
