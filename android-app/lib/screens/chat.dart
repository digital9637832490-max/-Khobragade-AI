...
class _ChatScreenState extends State<ChatScreen>{
 ...
 @override void dispose(){clock?.cancel();speech.stop();tts.stop();voicePhase.dispose();voiceWords.dispose();input.dispose();scroll.dispose();super.dispose();}
 ...
 Future<void> mic([bool autoSend=false,int retry=0])async{if(listening){await speech.stop();if(mounted)setState(()=>listening=false);await Future.delayed(const Duration(milliseconds:60));}voicePhase.value='listening';final ok=await speech.initialize(onStatus:(status){final active=status=='listening';if(active)voicePhase.value='listening';if(mounted)setState(()=>listening=active);if(autoSend&&voiceMode&&(status=='done'||status=='notListening')&&!busy&&retry<3){Future.delayed(const Duration(milliseconds:280),()=>mic(true,retry+1));}},onError:(e){if(mounted)setState(()=>listening=false);voicePhase.value='error';if(autoSend&&voiceMode&&!busy&&retry<3){Future.delayed(const Duration(milliseconds:420),()=>mic(true,retry+1));}});if(!ok){voicePhase.value='error';if(mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Microphone permission / speech service unavailable')));if(autoSend&&voiceMode&&retry<3)Future.delayed(const Duration(milliseconds:500),()=>mic(true,retry+1));return;}if(mounted)setState(()=>listening=true);await speech.listen(localeId:'hi_IN',listenFor:const Duration(seconds:45),pauseFor:const Duration(milliseconds:1100),partialResults:true,onResult:(r){input.text=r.recognizedWords;input.selection=TextSelection.collapsed(offset:input.text.length);voiceWords.value=r.recognizedWords;if(mounted)setState((){});if(r.finalResult&&autoSend){final said=r.recognizedWords.trim();if(said.isNotEmpty){speech.stop();if(mounted)setState(()=>listening=false);voicePhase.value='thinking';send(said,true);}}});}
 Future<void> fresh()async{messages=[];await tts.stop();await save();if(mounted)setState((){});}
 Future<void> stopVoiceConversation()async{voiceMode=false;voicePhase.value='ready';voiceWords.value='';await speech.stop();await tts.stop();await save();if(mounted)setState((){});}
 Future<void> openVoiceConversation()async{if(voiceMode)return;voiceMode=true;voicePhase.value='listening';voiceWords.value='';await save();if(mounted)setState((){});Future.microtask(()=>mic(true));if(!mounted)return;await Navigator.of(context).push(MaterialPageRoute<void>(fullscreenDialog:true,builder:(_)=>_KhobragadeVoicePage(phase:voicePhase,words:voiceWords,gender:voiceGender)));if(voiceMode)await stopVoiceConversation();}
