...
try{maintenance=await api.request('/system/status');}catch(_){}try{final w=await api.request('/wallet');coins=(w['coinBalance']??0) as int;}catch(_){}if(mounted)setState((){});}
 ...
 String spokenText(String text)=>text.replaceAll(RegExp(r'\n\s*\[\[SOURCES\]\][\s\S]*$'), '').trim();
 bool wantsSearch(String t){final x=t.toLowerCase();return RegExp(r'google|search|internet|web|latest|news|online|सर्च|खोज|गूगल|इंटरनेट|वेब|ताजा|लेटेस्ट|न्यूज़').hasMatch(x);}
 ...
 Future<Map<String,dynamic>> _clientContext()async{
  ...
  try{final info=Map<String,dynamic>.from(await _deviceChannel.invokeMethod('getDeviceInfo') ?? {});out.addAll({'deviceManufacturer':'${info['manufacturer']??''}','deviceModel':'${info['model']??''}','deviceBrand':'${info['brand']??''}','deviceAndroidVersion':'${info['androidVersion']??''}'});}catch(_){}
  try{
 ...
 Widget build(BuildContext context){
  ...
  itemBuilder:(_){
   ...
   body:Column(children:[
    Expanded(child:messages.isEmpty?_welcome():ListView.builder(controller:scroll,padding:const EdgeInsets.all(14),itemCount:messages.length+(busy?1:0),itemBuilder:(c,i)=>i==messages.length?_bubble({'role':'assistant','content':busyLabel}):_bubble(messages[i]))),
    _composer(),
   ]),
  );
 }
 ...
 Widget _bubble(Map<String,String> m){final user=m['role']=='user';final content=m['content']??'';Widget body;if(content.startsWith('[[IMAGE]]')){final d=content.substring(9);try{final b64=d.split(',').last;body=ClipRRect(borderRadius:BorderRadius.circular(12),child:Image.memory(base64Decode(b64),fit:BoxFit.contain));}catch(_){body=const Text('Image preview unavailable');}}else if(content.startsWith('[[VIDEO]]')){final raw=content.substring(9);final sep=raw.indexOf('|');final jobId=sep>0?raw.substring(0,sep):'';final url=sep>0?raw.substring(sep+1):raw;body=_VideoBubble(jobId:jobId,fallbackUrl:url);}else{final marker=content.indexOf('\n\n[[SOURCES]]');final mainText=marker>=0?content.substring(0,marker):content;final sourceText=marker>=0?content.substring(marker+13).trim():'';body=Column(crossAxisAlignment:CrossAxisAlignment.start,children:[SelectableText(mainText,style:TextStyle(color:user?Colors.white:Colors.black87,height:1.45)),if(!user&&sourceText.isNotEmpty)...[const SizedBox(height:10),const Text('Sources',style:TextStyle(fontWeight:FontWeight.w700)),SelectableText(sourceText,style:const TextStyle(fontSize:12,color:Colors.blue,height:1.35))]]);}return Align(alignment:user?Alignment.centerRight:Alignment.centerLeft,child:Container(margin:const EdgeInsets.only(bottom:14),constraints:const BoxConstraints(maxWidth:620),padding:const EdgeInsets.all(14),decoration:BoxDecoration(color:user?Colors.black:Colors.white,border:user?null:Border.all(color:Colors.blue.shade100),borderRadius:BorderRadius.circular(18)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[body,if(!user&&!content.startsWith('[[IMAGE]]')&&!content.startsWith('[[VIDEO]]'))Row(mainAxisSize:MainAxisSize.min,children:[IconButton(onPressed:()=>speak(spokenText(content)),icon:const Icon(Icons.volume_up,size:18)),IconButton(onPressed:tts.stop,icon:const Icon(Icons.stop,size:18))])])));}
...
class _VideoBubble extends StatefulWidget {
  ...
  const _VideoBubble({required this.jobId,required this.fallbackUrl});
  @override State<_VideoBubble> createState()=>_VideoBubbleState();
}
...
class _VideoBubbleState extends State<_VideoBubble>{
  ...
  Future<void> _load()async{
    try{
      ...
      final url=widget.jobId.isNotEmpty?'${Config.apiBaseUrl}/ai/video/${widget.jobId}/file':widget.fallbackUrl;
      final c=VideoPlayerController.networkUrl(Uri.parse(url),httpHeaders:{if(token!=null)'Authorization':'Bearer $token'});
      await c.initialize();
  ...
  @override Widget build(BuildContext context){
    final c=controller;
    if(c==null)return const Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('🎬 Video ready'),SizedBox(height:8),CircularProgressIndicator(strokeWidth:2)]);
    return Column(crossAxisAlignment:CrossAxisAlignment.start,children:[AspectRatio(aspectRatio:c.value.aspectRatio>0?c.value.aspectRatio:16/9,child:ClipRRect(borderRadius:BorderRadius.circular(12),child:VideoPlayer(c))),Row(children:[IconButton(onPressed:(){setState((){c.value.isPlaying?c.pause():c.play();});},icon:ValueListenableBuilder<VideoPlayerValue>(valueListenable:c,builder:(context,v,_)=>Icon(v.isPlaying?Icons.pause:Icons.play_arrow))),const Text('Video ready')])]);
  }
}
