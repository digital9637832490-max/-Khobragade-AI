...
try{maintenance=await api.request('/system/status');}catch(_){}try{final w=await api.request('/wallet');coins=(w['coinBalance']??0) as int;}catch(_){}if(mounted)setState((){});}
 ...
 Future<void> openVoiceConversation()async{if(voiceMode)return;voiceMode=true;voicePhase.value='listening';voiceWords.value='';await speech.stop();await tts.stop();await save();liveVoice=LiveVoiceSession(onPhase:(p){if(mounted)voicePhase.value=p;},onWords:(w){if(mounted)voiceWords.value=w;},onError:(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(e)));});try{final ctx=await _clientContext();await liveVoice!.start(voiceGender,ctx);}catch(e){voiceMode=false;voicePhase.value='error';if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(e.toString().replaceFirst('Exception: ',''))));return;}if(!mounted)return;await Navigator.of(context).push(MaterialPageRoute<void>(fullscreenDialog:true,builder:(_)=>_KhobragadeVoicePage(phase:voicePhase,words:voiceWords,gender:voiceGender)));if(voiceMode)await stopVoiceConversation();}
