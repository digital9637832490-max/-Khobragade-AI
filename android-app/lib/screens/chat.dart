...
class _GeneratedVideo extends StatefulWidget {
  final String url;
  const _GeneratedVideo({required this.url});
  @override State<_GeneratedVideo> createState()=>_GeneratedVideoState();
}
class _GeneratedVideoState extends State<_GeneratedVideo> {
  VideoPlayerController? _controller; String? _error;
  @override void initState(){super.initState();_load();}
  Future<void> _load()async{
    try{final p=await SharedPreferences.getInstance();final token=p.getString('token');final c=VideoPlayerController.networkUrl(Uri.parse(widget.url),httpHeaders:{if(token!=null&&token.isNotEmpty)'Authorization':'Bearer $token'});await c.initialize();if(!mounted){await c.dispose();return;}setState(()=>_controller=c);}
    catch(_){if(mounted)setState(()=>_error='Video preview failed. Tap retry.');}
  }
  @override void dispose(){_controller?.dispose();super.dispose();}
  @override Widget build(BuildContext context){
    if(_error!=null)return Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(_error!),TextButton(onPressed:(){setState((){_error=null;_controller=null;});_load();},child:const Text('Retry'))]);
    final c=_controller;if(c==null)return const Padding(padding:EdgeInsets.all(16),child:Row(mainAxisSize:MainAxisSize.min,children:[SizedBox(width:18,height:18,child:CircularProgressIndicator(strokeWidth:2)),SizedBox(width:10),Text('Video loading…')]));
    return Column(crossAxisAlignment:CrossAxisAlignment.start,children:[ClipRRect(borderRadius:BorderRadius.circular(12),child:AspectRatio(aspectRatio:c.value.aspectRatio>0?c.value.aspectRatio:16/9,child:VideoPlayer(c))),Row(mainAxisSize:MainAxisSize.min,children:[IconButton(onPressed:(){setState((){c.value.isPlaying?c.pause():c.play();});},icon:Icon(c.value.isPlaying?Icons.pause_circle:Icons.play_circle)),IconButton(onPressed:()async{await c.seekTo(Duration.zero);if(!c.value.isPlaying)await c.play();},icon:const Icon(Icons.replay))])]);
  }
}
