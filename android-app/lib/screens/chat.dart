...
class _ChatScreenState extends State<ChatScreen>{
 ...
 Future<void> speak(String text,{bool continueVoice=false})async{
  final clean=_ttsText(text);if(clean.isEmpty)return;voicePhase.value='speaking';ttsSpeaking=true;bargeInListening=false;
  await tts.stop();await tts.awaitSpeakCompletion(true);await tts.setLanguage(language=='mr'?'mr-IN':language=='hi'?'hi-IN':'en-IN');await tts.setSpeechRate(.48);
  final voices=await tts.getVoices;try{
    final list=List<Map>.from(voices as List);availableVoices=List<Map<String,dynamic>>.from(list.map((v)=>Map<String,dynamic>.from(v)));Map<String,dynamic>? chosen;
    final langCode=language=='mr'?'mr':language=='en'?'en':'hi';
    if(voiceName.isNotEmpty){for(final v in availableVoices){if('${v['name']}'==voiceName&&'${v['locale']}'.toLowerCase().startsWith(langCode)){chosen=v;break;}}}
    if(chosen==null){final wanted=availableVoices.where((v){final n='${v['name']}'.toLowerCase(),l='${v['locale']}'.toLowerCase();if(!l.startsWith(langCode))return false;return voiceGender=='female'?RegExp('female|heera|swara|veena|zira|samantha|hindi.*f|x-hia|x-hic').hasMatch(n):RegExp('male|ravi|hemant|david|mark|hindi.*m|x-hid|x-hie').hasMatch(n);}).toList();if(wanted.isNotEmpty)chosen=wanted.first;}
    if(chosen==null){for(final v in availableVoices){if('${v['locale']}'.toLowerCase().startsWith(langCode)){chosen=v;break;}}}
    if(chosen!=null){voiceName='${chosen['name']}';await tts.setVoice({'name':'${chosen['name']}','locale':'${chosen['locale']}'});}
  }catch(_){}
  if(voiceMode&&mounted){bargeInListening=true;try{await speech.initialize(onStatus:(_){},onError:(_){ });await speech.listen(listenOptions:stt.SpeechListenOptions(localeId:language=='mr'?'mr-IN':language=='en'?'en-IN':'hi-IN',listenFor:const Duration(seconds:60),pauseFor:const Duration(seconds:2),partialResults:true,listenMode:stt.ListenMode.dictation,cancelOnError:false),onResult:(r){final words=r.recognizedWords.trim();if(words.length>=2&&ttsSpeaking){ttsSpeaking=false;bargeInListening=false;pendingVoiceSubmit=true;unawaited(tts.stop());lastVoiceText=words;voiceWords.value=words;voicePhase.value='listening';if(mounted)setState(()=>listening=true);if(r.finalResult&&!voiceSending&&!busy)unawaited(_submitVoiceWords());}});}catch(_){bargeInListening=false;}}
  try{await tts.speak(clean);}finally{ttsSpeaking=false;if(bargeInListening){bargeInListening=false;await speech.stop();}}
  if(continueVoice&&voiceMode&&mounted){await Future<void>.delayed(const Duration(milliseconds:120));if(voiceMode&&mounted&&!busy&&!voiceSending){voiceRestarting=true;voicePhase.value='listening';await mic(true,0);voiceRestarting=false;}}else if(voiceMode&&mounted&&!voiceSending){voicePhase.value='listening';}
 }
 ...
 Future<void> send([String? value,bool speakReply=false])async{
  final text=(value??input.text).trim();if((text.isEmpty&&attachmentData==null)||busy)return;FocusScope.of(context).unfocus();
  final sentText=text.isEmpty?'Attached file: ${attachmentName??'file'}':text;setState((){messages.add({'role':'user','content':attachmentName==null?sentText:'$sentText\n📎 $attachmentName'});input.clear();busy=true;});await save();
  try{Map<String,dynamic>? job;String spoken='';
    if(wantsVideo(sentText)){voicePhase.value='thinking';final ctx=await _clientContext();String? previousImage;for(final m in messages.reversed){final c=m['content']??'';if(m['role']=='assistant'&&c.startsWith('[[IMAGE]]')){previousImage=c.substring(9);break;}}job=await api.request('/ai/video',method:'POST',body:{'prompt':sentText,if(previousImage!=null)'imageDataUrl':previousImage,...ctx});final r=await waitJob(job['id'].toString(),video:true);final uri=(r['videoUri']??r['videoUrl']??'').toString();final answer=uri.isEmpty?'वीडियो generate हो गया।':'[[VIDEO]]$uri';setState(()=>messages.add({'role':'assistant','content':answer}));spoken='वीडियो तैयार हो गया है।';}
    else if(wantsImage(sentText)){voicePhase.value='thinking';job=await api.request('/ai/photo',method:'POST',body:{'prompt':sentText});final r=await waitJob(job['id'].toString());final img=(r['imageDataUrl']??r['imageUrl']??'').toString();if(img.isEmpty)throw Exception('Image generated but image data missing');setState(()=>messages.add({'role':'assistant','content':'[[IMAGE]]$img'}));spoken='इमेज तैयार हो गई है।';}
    else{voicePhase.value='thinking';final history=messages.length>20?messages.sublist(messages.length-20):messages;final ctx=await _clientContext();final r=await api.request('/ai/chat',method:'POST',body:{'message':sentText,'history':history,'voiceGender':voiceGender,'language':language,...ctx,if(attachmentData!=null)'attachmentName':attachmentName,if(attachmentData!=null)'attachmentMime':attachmentMime,if(attachmentData!=null)'attachmentData':attachmentData});final answer=(r['answer']??r['description']??r['text']??'').toString().trim();if(answer.isEmpty)throw Exception('AI returned an empty response');setState(()=>messages.add({'role':'assistant','content':answer}));spoken=answer;}
    if((speakReply||voiceMode)&&spoken.isNotEmpty)await speak(spoken,continueVoice:false);if(mounted)setState((){attachmentName=null;attachmentMime=null;attachmentData=null;});
  }catch(e){final raw=e.toString().replaceFirst('Exception: ','');String msg='⚠️ $raw';if(raw.contains('ALL_IMAGE_PROVIDERS_EXHAUSTED'))msg='⚠️ अभी image generation की सभी configured AI services उपलब्ध नहीं हैं। कृपया थोड़ी देर बाद फिर कोशिश करें।';else if(raw.contains('IMAGE_PROVIDER_BILLING_REQUIRED'))msg='⚠️ Image generation service के लिए provider access/billing चाहिए।';else if(raw.contains('ALL_AI_PROVIDERS_EXHAUSTED'))msg='⚠️ अभी सभी configured AI services उपलब्ध नहीं हैं। कृपया थोड़ी देर बाद फिर कोशिश करें।';else if(raw.contains('GEMINI_DAILY_QUOTA')){quotaKind='daily';quotaUntil=DateTime.now().add(const Duration(hours:24));msg='आज की AI उपयोग सीमा पूरी हो गई है। अगले quota reset के बाद फिर कोशिश करें।';}else if(raw.contains('VIDEO_PROVIDER_BILLING_REQUIRED'))msg='⚠️ Video generation ke liye Google billing/model access chahiye.';else if(raw.contains('GEMINI_RATE_LIMIT')||raw.contains('429')){quotaKind='minute';quotaUntil=DateTime.now().add(const Duration(minutes:1));msg='अभी बहुत requests आ गई हैं। थोड़ी देर बाद फिर कोशिश करें।';}setState(()=>messages.add({'role':'assistant','content':msg}));}
  finally{busy=false;await save();if(mounted)setState((){});if(pendingVoiceSubmit&&voiceMode&&!voiceSending){pendingVoiceSubmit=false;Future.microtask(_submitVoiceWords);}Future.delayed(const Duration(milliseconds:40),()=>scroll.hasClients?scroll.animateTo(scroll.position.maxScrollExtent,duration:const Duration(milliseconds:140),curve:Curves.easeOut):null);}
 }
 Future<void> _submitVoiceWords()async{if(voiceSending||busy||!voiceMode)return;final said=lastVoiceText.trim();if(said.isEmpty)return;voiceSending=true;lastVoiceText='';bargeInListening=false;await speech.stop();if(mounted)setState(()=>listening=false);voicePhase.value='thinking';try{await send(said,true);}finally{voiceSending=false;if(voiceMode&&mounted&&!busy&&!voiceRestarting){voiceRestarting=true;try{await Future.delayed(const Duration(milliseconds:180));if(voiceMode&&mounted&&!busy&&!voiceSending){voicePhase.value='listening';await mic(true,0);}}finally{voiceRestarting=false;}}}}
 ...
 String _voiceLabel(Map<String,dynamic> v){final l='${v['locale']??''}'.toLowerCase();final n='${v['name']??''}';final lang=l.startsWith('mr')?'Marathi':l.startsWith('hi')?'Hindi':l.startsWith('en')?'English':'Voice';final female=RegExp('female|heera|swara|veena|zira|samantha|hindi.*f|x-hia|x-hic',caseSensitive:false).hasMatch(n);final male=RegExp('male|ravi|hemant|david|mark|hindi.*m|x-hid|x-hie',caseSensitive:false).hasMatch(n);return '$lang ${female?'Female':male?'Male':'Voice'}';}
 ...
 Widget build(BuildContext context){
  ...
  itemBuilder:(_){
    ...
    final matching=availableVoices.where((v){
      ...
      if(!(l.startsWith('hi')||l.startsWith('en')||l.startsWith('mr')))return false;
    ...
    for(final v in matching){final n='${v['name']}';if(n.isNotEmpty&&unique.add(n))items.add(PopupMenuItem(value:n,child:Text(_voiceLabel(v),maxLines:1,overflow:TextOverflow.ellipsis)));}
