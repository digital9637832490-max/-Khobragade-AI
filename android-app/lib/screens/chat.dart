...
try{maintenance=await api.request('/system/status');}catch(_){}try{final w=await api.request('/wallet');coins=(w['coinBalance']??0) as int;}catch(_){}if(mounted)setState((){});}
 ...
 Future<void> setAttachmentBytes(String name,List<int> bytes)async{
  ...
  final mime={'jpg':'image/jpeg','jpeg':'image/jpeg','png':'image/png','webp':'image/webp','pdf':'application/pdf','txt':'text/plain','csv':'text/csv','doc':'application/msword','docx':'application/vnd.openxmlformats-officedocument.wordprocessingml.document','xls':'application/vnd.ms-excel','xlsx':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','mp3':'audio/mpeg','wav':'audio/wav','m4a':'audio/mp4','mp4':'video/mp4','mov':'video/quicktime','webm':'video/webm'}[ext]??'application/octet-stream';
 ...
 Future<void> pickFromFiles()async{
  final r=await FilePicker.platform.pickFiles(withData:true,allowMultiple:false,type:FileType.custom,allowedExtensions:['jpg','jpeg','png','webp','pdf','txt','csv','doc','docx','xls','xlsx','mp3','wav','m4a','mp4','mov','webm']);
 ...
 Future<Map<String,dynamic>> _clientContext()async{
  ...
  final out=<String,dynamic>{'localDateTime':now.toIso8601String(),'utcNow':now.toUtc().toIso8601String(),'timeZone':now.timeZoneName};
  try{final tz=await _deviceChannel.invokeMethod<String>('getTimeZone');if(tz!=null&&tz.trim().isNotEmpty)out['timeZone']=tz.trim();}catch(_){}
  try{final info=Map<String,dynamic>.from(await _deviceChannel.invokeMethod('getDeviceInfo') ?? {});out.addAll({'deviceManufacturer':'${info['manufacturer']??''}','deviceModel':'${info['model']??''}','deviceBrand':'${info['brand']??''}','deviceAndroidVersion':'${info['androidVersion']??''}','batteryPercent':info['batteryPercent']??-1});}catch(_){}
  try{
    ...
    if(enabled && permission!=LocationPermission.denied && permission!=LocationPermission.deniedForever){
      Position? pos;
      try{pos=await Geolocator.getCurrentPosition(locationSettings:const LocationSettings(accuracy:LocationAccuracy.high,timeLimit:Duration(seconds:8)));}catch(_){try{pos=await Geolocator.getLastKnownPosition();}catch(_){}}
      if(pos!=null){out['latitude']=pos.latitude;out['longitude']=pos.longitude;out['locationAccuracyMeters']=pos.accuracy;try{final loc=Map<String,dynamic>.from(await _deviceChannel.invokeMethod('reverseGeocode',{'latitude':pos.latitude,'longitude':pos.longitude}) ?? {});final label='${loc['formatted']??''}'.trim();if(label.isNotEmpty)out['locationLabel']=label;}catch(_){}}
    }
  }catch(_){ }
  return out;
 }
 bool wantsDocument(String t){final x=t.toLowerCase();return RegExp(r'\b(pdf|document|docx|file)\b|पीडीएफ|दस्तावेज|डॉक्यूमेंट',caseSensitive:false).hasMatch(x) && RegExp(r'make|create|generate|banao|bana do|banado|बना|बनाओ|बनाएं|तैयार',caseSensitive:false).hasMatch(x);}
 Future<void> createPdfFromText(String title,String text)async{final r=await api.request('/documents/pdf',method:'POST',body:{'title':title,'text':text});final bytes=base64Decode(String(r['dataBase64']||''));final dir=await getApplicationDocumentsDirectory();final file=File('${dir.path}/${r['fileName']||'khobragade_ai.pdf'}');await file.writeAsBytes(bytes,flush:true);if(mounted)setState(()=>messages.add({'role':'assistant','content':'[[FILE]]${file.path}'}));}
 Future<void> send([String? value,bool speakReply=false])async{final text=(value??input.text).trim();if((text.isEmpty&&attachmentData==null)||busy)return;final sentText=text.isEmpty?'Attached file: ${attachmentName??'file'}':text;setState((){messages.add({'role':'user','content':attachmentName==null?sentText:'$sentText\n📎 $attachmentName'});input.clear();busy=true;});await save();try{Map<String,dynamic> job;String spoken='';if(wantsDocument(sentText)){voicePhase.value='thinking';final history=messages.length>20?messages.sublist(messages.length-20):messages;final ctx=await _clientContext();job=await api.request('/ai/chat',method:'POST',body:{'message':'Create the document content requested by the user. Return clean plain text with headings and no markdown fences. '+sentText,'history':history,'voiceGender':voiceGender,...ctx});final r=await waitJob(job['id'].toString());final content=(r['answer']??'').toString();await createPdfFromText('Khobragade AI Document',content);spoken='PDF तैयार हो गई है।';}else if(wantsVideo(sentText)){voicePhase.value='thinking';final ctx=await _clientContext();String? previousImage;for(final m in messages.reversed){final c=m['content']??'';if(m['role']=='assistant'&&c.startsWith('[[IMAGE]]')){previousImage=c.substring(9);break;}}final attachedImage=(attachmentData!=null&&(attachmentMime??'').startsWith('image/'))?'data:${attachmentMime};base64,$attachmentData':null;final sourceImage=previousImage??attachedImage;job=await api.request('/ai/video',method:'POST',body:{'prompt':sentText,if(sourceImage!=null)'imageDataUrl':sourceImage,...ctx});final r=await waitJob(job['id'].toString(),video:true);final uri=(r['videoUri']??r['videoUrl']??'').toString();final answer=uri.isEmpty?'✅ Video generate ho gaya.':'[[VIDEO]]$uri';setState(()=>messages.add({'role':'assistant','content':answer}));spoken='वीडियो तैयार हो गया है।';}else if(wantsImage(sentText)){voicePhase.value='thinking';job=await api.request('/ai/photo',method:'POST',body:{'prompt':sentText,if(attachmentData!=null&&((attachmentMime??'').startsWith('image/'))) 'imageDataUrl':'data:${attachmentMime};base64,$attachmentData'});final r=await waitJob(job['id'].toString());final img=(r['imageDataUrl']??r['imageUrl']??'').toString();if(img.isEmpty)throw Exception('Image generated but image data missing');setState(()=>messages.add({'role':'assistant','content':'[[IMAGE]]$img'}));spoken='इमेज तैयार हो गई है।';}else{voicePhase.value='thinking';final history=messages.length>20?messages.sublist(messages.length-20):messages;final ctx=await _clientContext();job=await api.request('/ai/chat',method:'POST',body:{'message':sentText,'history':history,'voiceGender':voiceGender,...ctx,if(attachmentData!=null)'attachmentName':attachmentName,if(attachmentData!=null)'attachmentMime':attachmentMime,if(attachmentData!=null)'attachmentData':attachmentData});final r=await waitJob(job['id'].toString());final answer=(r['answer']??r['description']??'').toString();final src=(r['sources'] is List)?List<Map<String,String>>.from((r['sources'] as List).whereType<Map>().map((x)=>{'title':'${x['title']??''}','uri':'${x['uri']??''}'})):<Map<String,String>>[];final sourceText=src.isEmpty?'': '\n\nSources:\n'+src.asMap().entries.map((e)=>'[${e.key+1}] ${e.value['title']}\n${e.value['uri']}').join('\n');setState(()=>messages.add({'role':'assistant','content':answer+sourceText}));spoken=answer;}if((speakReply||voiceMode)&&spoken.isNotEmpty)await speak(spoken,continueVoice:false);if(mounted)setState((){attachmentName=null;attachmentMime=null;attachmentData=null;});final w=await api.request('/wallet');coins=(w['coinBalance']??coins) as int;}catch(e){final raw=e.toString().replaceFirst('Exception: ','');String msg='⚠️ $raw';if(raw.contains('IMAGE_PROVIDER_BILLING_REQUIRED'))msg='⚠️ Image generation model ko Google billing/model access chahiye.';else if(raw.contains('GEMINI_DAILY_QUOTA')){quotaKind='daily';quotaUntil=DateTime.now().add(const Duration(hours:24));msg='आज की AI उपयोग सीमा पूरी हो गई है। अगले quota reset के बाद फिर कोशिश करें।';}else if(raw.contains('VIDEO_PROVIDER_BILLING_REQUIRED'))msg='⚠️ Video generation ke liye Google billing/model access chahiye.';else if(raw.contains('GEMINI_RATE_LIMIT')||raw.contains('429')){quotaKind='minute';quotaUntil=DateTime.now().add(const Duration(minutes:1));msg='अभी बहुत requests आ गई हैं। थोड़ी देर बाद फिर कोशिश करें।';}setState(()=>messages.add({'role':'assistant','content':msg}));}finally{busy=false;await save();if(mounted)setState((){});Future.delayed(const Duration(milliseconds:40),()=>scroll.hasClients?scroll.animateTo(scroll.position.maxScrollExtent,duration:const Duration(milliseconds:140),curve:Curves.easeOut):null);}}
 ...
 Future<void> openVoiceConversation()async{if(voiceMode)return;voiceMode=true;voicePhase.value='listening';voiceWords.value='';await speech.stop();await tts.stop();await save();liveVoice=LiveVoiceSession(onPhase:(p){if(mounted)voicePhase.value=p;},onWords:(w){if(mounted)voiceWords.value=w;},onError:(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(e)));});try{final ctx=await _clientContext();await liveVoice!.start(voiceGender,context:ctx);}catch(e){voiceMode=false;voicePhase.value='error';if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(e.toString().replaceFirst('Exception: ',''))));return;}if(!mounted)return;await Navigator.of(context).push(MaterialPageRoute<void>(fullscreenDialog:true,builder:(_)=>_KhobragadeVoicePage(phase:voicePhase,words:voiceWords,gender:voiceGender)));if(voiceMode)await stopVoiceConversation();}
