...
server.on('upgrade', (req, socket, head) => {
    try {
        ...
        req.voiceGender = u.searchParams.get('gender') === 'male' ? 'male' : 'female';
        req.voiceContext = {
...
liveWss.on('connection', (client, req) => {
    const apiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_GEMINI_API_KEY || '';
    if (!apiKey) {
        client.close(1011, 'Gemini API key missing');
        return;
    }
    ...
    const gemini = new WebSocket(`wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1beta.GenerativeService.BidiGenerateContent?key=${encodeURIComponent(apiKey)}`);
    let ready = false;
    const pending = [];
    gemini.on('open', () => {
    ...
    gemini.on('close', (c, r) => { if (client.readyState === WebSocket.OPEN) {
        const reason = r.toString().slice(0, 180) || `Live AI closed (${c})`;
        try {
            client.send(JSON.stringify({ error: reason }));
        }
        catch { }
        client.close(c === 1000 ? 1000 : 1011, reason);
    } });
    gemini.on('error', (err) => { console.error('Gemini Live error:', err); if (client.readyState === WebSocket.OPEN) {
        try {
            client.send(JSON.stringify({ error: 'Live AI connection failed' }));
        }
        catch { }
        client.close(1011, 'Live AI connection failed');
    } });
    client.on('message', data => { if (gemini.readyState !== WebSocket.OPEN || !ready)
        pending.push(data);
    else
        gemini.send(data); });
    client.on('close', () => { if (gemini.readyState === WebSocket.OPEN || gemini.readyState === WebSocket.CONNECTING)
        gemini.close(); });
    client.on('error', () => { try {
        gemini.close();
    }
