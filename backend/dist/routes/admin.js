...
adminRouter.post('/login', async (req, res, next) => {
    try {
        ...
        const a = q.rows[0];
        if (!a || !(await bcrypt.compare(b.password, a.password_hash)))
...
adminRouter.get('/users', async (_req, res, next) => {
    try {
        const q = await pool.query('SELECT id,name,email,coin_balance,status,created_at FROM users ORDER BY created_at DESC LIMIT 500');
        res.json(q.rows);
    }
    catch (e) {
        next(e);
...
adminRouter.get('/payments', async (_req, res, next) => {
    try {
        const q = await pool.query(`SELECT p.*,u.name,u.email,c.name package_name,c.coins,c.bonus_coins FROM payment_requests p JOIN users u ON u.id=p.user_id JOIN coin_packages c ON c.id=p.package_id ORDER BY p.created_at DESC`);
        res.json(q.rows);
    }
    catch (e) {
        next(e);
...
adminRouter.get('/audit-logs', async (_req, res, next) => {
    try {
        const q = await pool.query('SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT 500');
        res.json(q.rows);
    }
    catch (e) {
        next(e);
...
adminRouter.get('/coin-packages', async (_req, res, next) => { try {
    const q = await pool.query('SELECT * FROM coin_packages ORDER BY sort_order,price_inr');
    res.json(q.rows);
}
catch (e) {
    next(e);
} });
adminRouter.post('/coin-packages', async (req, res, next) => { try {
    const b = z.object({ name: z.string().min(1), coins: z.number().int().positive(), bonusCoins: z.number().int().nonnegative().default(0), priceInr: z.number().nonnegative(), isActive: z.boolean().default(true) }).parse(req.body);
    const q = await pool.query('INSERT INTO coin_packages(name,coins,bonus_coins,price_inr,is_active) VALUES($1,$2,$3,$4,$5) RETURNING *', [b.name, b.coins, b.bonusCoins, b.priceInr, b.isActive]);
    await audit(req.auth.id, 'package.create', 'coin_package', q.rows[0].id, b);
    res.status(201).json(q.rows[0]);
}
catch (e) {
    next(e);
} });
adminRouter.patch('/coin-packages/:id', async (req, res, next) => { try {
    const b = z.object({ name: z.string().min(1), coins: z.number().int().positive(), bonusCoins: z.number().int().nonnegative(), priceInr: z.number().nonnegative(), isActive: z.boolean() }).parse(req.body);
    const q = await pool.query('UPDATE coin_packages SET name=$2,coins=$3,bonus_coins=$4,price_inr=$5,is_active=$6,updated_at=now() WHERE id=$1 RETURNING *', [req.params.id, b.name, b.coins, b.bonusCoins, b.priceInr, b.isActive]);
    await audit(req.auth.id, 'package.update', 'coin_package', req.params.id, b);
    res.json(q.rows[0]);
}
catch (e) {
    next(e);
} });
adminRouter.delete('/coin-packages/:id', async (req, res, next) => { try {
    await pool.query('UPDATE coin_packages SET is_active=false,updated_at=now() WHERE id=$1', [req.params.id]);
    await audit(req.auth.id, 'package.disable', 'coin_package', req.params.id);
    res.json({ ok: true });
}
catch (e) {
    next(e);
} });
adminRouter.get('/projects', async (_req, res, next) => { try {
    const q = await pool.query(`SELECT p.*,u.name user_name,u.email FROM projects p JOIN users u ON u.id=p.user_id ORDER BY p.created_at DESC LIMIT 500`);
    res.json(q.rows);
}
catch (e) {
    next(e);
} });
adminRouter.get('/jobs', async (_req, res, next) => { try {
    const q = await pool.query(`SELECT j.*,u.name user_name,u.email FROM ai_jobs j JOIN users u ON u.id=j.user_id ORDER BY j.created_at DESC LIMIT 500`);
    res.json(q.rows);
}
catch (e) {
    next(e);
} });
adminRouter.delete('/jobs/:id', async (req, res, next) => { try {
    await pool.query(`DELETE FROM ai_jobs WHERE id=$1 AND status IN ('failed','completed')`, [req.params.id]);
...
adminRouter.post('/jobs/:id/block', async (req, res, next) => { try {
    await pool.query(`UPDATE ai_jobs SET status='failed',error_message='Blocked by admin',completed_at=now() WHERE id=$1 AND status IN ('pending','processing')`, [req.params.id]);
...
adminRouter.post('/notifications', async (req, res, next) => { try {
    const b = z.object({ userId: z.string().uuid().nullable().optional(), title: z.string().min(1), body: z.string().min(1), type: z.string().default('info') }).parse(req.body);
    const q = await pool.query('INSERT INTO notifications(user_id,title,body,type) VALUES($1,$2,$3,$4) RETURNING *', [b.userId || null, b.title, b.body, b.type]);
...
adminRouter.get('/support', async (_req, res, next) => { try {
    const q = await pool.query(`SELECT t.*,u.name user_name,u.email FROM support_tickets t JOIN users u ON u.id=t.user_id ORDER BY t.created_at DESC`);
    res.json(q.rows);
}
catch (e) {
    next(e);
} });
adminRouter.post('/support/:id/reply', async (req, res, next) => { try {
    const b = z.object({ message: z.string().min(1) }).parse(req.body);
    await tx(async (c) => { await c.query('INSERT INTO support_messages(ticket_id,sender_role,sender_id,message) VALUES($1,\'admin\',$2,$3)', [req.params.id, req.auth.id, b.message]); await c.query('UPDATE support_tickets SET admin_reply=$2,updated_at=now() WHERE id=$1', [req.params.id, b.message]); });
...
adminRouter.post('/support/:id/status', async (req, res, next) => { try {
    const b = z.object({ status: z.enum(['open', 'closed']) }).parse(req.body);
    await pool.query('UPDATE support_tickets SET status=$2,updated_at=now() WHERE id=$1', [req.params.id, b.status]);
...
adminRouter.get('/settings', async (_req, res, next) => { try {
    const q = await pool.query('SELECT * FROM settings ORDER BY key');
    res.json(q.rows);
}
catch (e) {
    next(e);
