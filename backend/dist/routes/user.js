...
userRouter.get('/time', async (req, res, next) => {
    try {
        ...
        const validTimeZone = (() => { try {
            new Intl.DateTimeFormat('en-IN', { timeZone: tz }).format(date);
            return tz;
        }
        catch {
            return 'Asia/Kolkata';
        ...
        if (validTimeZone !== tz) {
            parts = new Intl.DateTimeFormat('en-IN', { timeZone: validTimeZone, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).formatToParts(date);
        }
        res.json({ iso: date.toISOString(), timeZone: validTimeZone, localDateTime: `${get('year')}-${get('month')}-${get('day')}T${get('hour')}:${get('minute')}:${get('second')}` });
    }
    catch (e) {
        next(e);
    }
...
userRouter.get('/wallet', async (req, res, next) => {
    try {
        const q = await pool.query('SELECT coin_balance FROM users WHERE id=$1', [req.auth.id]);
        res.json({ coinBalance: Number(q.rows[0]?.coin_balance || 0) });
    }
    catch (e) {
        next(e);
    }
...
userRouter.get('/wallet/transactions', async (req, res, next) => {
    try {
        const q = await pool.query('SELECT * FROM wallet_transactions WHERE user_id=$1 ORDER BY created_at DESC LIMIT 200', [req.auth.id]);
        res.json(q.rows);
    }
    catch (e) {
        next(e);
...
userRouter.get('/coin-packages', async (_req, res, next) => {
    try {
        const q = await pool.query('SELECT * FROM coin_packages WHERE is_active=true ORDER BY sort_order, price_inr');
        res.json(q.rows);
    }
    catch (e) {
        next(e);
...
userRouter.post('/payments/request', async (req, res, next) => {
    try {
        ...
        const q = await pool.query(`INSERT INTO payment_requests(user_id,package_id,amount_inr,transaction_id,proof_file_key)
       VALUES($1,$2,$3,$4,$5) RETURNING *`, [req.auth.id, b.packageId, p.rows[0].price_inr, b.transactionId, b.proofFileKey || null]);
...
userRouter.get('/projects', async (req, res, next) => {
    try {
        const q = await pool.query('SELECT * FROM projects WHERE user_id=$1 ORDER BY created_at DESC', [req.auth.id]);
        res.json(q.rows);
    }
    catch (e) {
        next(e);
...
async function createAiJob(userId, toolKey, input) {
    return tx(async (c) => {
        ...
        if (!cfg.enabled || cfg.maintenance)
        ...
        if (toolKey !== 'chat') {
            ...
            if (Number(cfg.dailyLimit || 0) > 0 && usedToday >= Number(cfg.dailyLimit))
        ...
        if (cost > 0)
            await changeCoins(c, userId, -cost, `ai:${toolKey}`, 'AI generation', j.rows[0].id);
        return j.rows[0];
    });
}
userRouter.post('/ai/voice-chat', async (req, res, next) => {
    try {
        const b = z.object({
...
userRouter.post('/ai/thumbnail', async (req, res, next) => { try {
    res.status(202).json(await createAiJob(req.auth.id, 'thumbnail', req.body));
}
catch (e) {
    next(e);
} });
userRouter.post('/ai/photo', async (req, res, next) => { try {
    res.status(202).json(await createAiJob(req.auth.id, 'photo', req.body));
}
catch (e) {
    next(e);
} });
userRouter.post('/ai/content', async (req, res, next) => {
    try {
...
userRouter.post('/ai/voiceover', async (req, res, next) => { try {
    res.status(202).json(await createAiJob(req.auth.id, 'voiceover', req.body));
}
catch (e) {
    next(e);
} });
userRouter.post('/ai/video', async (req, res, next) => { try {
    res.status(202).json(await createAiJob(req.auth.id, 'video', req.body));
}
catch (e) {
    next(e);
} });
userRouter.get('/jobs', async (req, res, next) => { try {
    const q = await pool.query('SELECT * FROM ai_jobs WHERE user_id=$1 ORDER BY created_at DESC LIMIT 200', [req.auth.id]);
    res.json(q.rows);
}
catch (e) {
    next(e);
...
userRouter.get('/notifications', async (req, res, next) => {
    try {
        const q = await pool.query('SELECT * FROM notifications WHERE user_id=$1 OR user_id IS NULL ORDER BY created_at DESC LIMIT 100', [req.auth.id]);
        res.json(q.rows);
    }
    catch (e) {
        next(e);
...
userRouter.get('/support', async (req, res, next) => { try {
    const q = await pool.query('SELECT * FROM support_tickets WHERE user_id=$1 ORDER BY created_at DESC', [req.auth.id]);
    res.json(q.rows);
}
catch (e) {
    next(e);
...
userRouter.post('/support/:id/reply', async (req, res, next) => { try {
    const b = z.object({ message: z.string().min(1) }).parse(req.body);
    const own = await pool.query('SELECT id FROM support_tickets WHERE id=$1 AND user_id=$2', [req.params.id, req.auth.id]);
    if (!own.rowCount)
        return res.status(404).json({ error: 'Not found' });
    await pool.query(`INSERT INTO support_messages(ticket_id,sender_role,sender_id,message) VALUES($1,'user',$2,$3)`, [req.params.id, req.auth.id, b.message]);
