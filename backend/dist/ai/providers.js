...
async function fetchWebContext(query) {
    ...
    const sources = [];
    const blocks = [];
    const add = (title, url, snippet) => { if (url && !sources.some(s => s.url === url)) {
        sources.push({ title: title || url, url });
        blocks.push(`- ${title || url}: ${snippet}`);
    } };
        ...
        if (newsLike) {
            ...
            if (r.ok) {
                ...
                for (const m of items) {
                    ...
                    if (title && link)
                        add(title, link, desc.slice(0, 500));
                }
            }
        }
    }
    catch (e) {
        console.warn('Google News RSS fallback failed', e);
    }
        ...
        if (tavily) {
            ...
            if (r.ok) {
                ...
                for (const x of (d.results || []))
                    add(String(x.title || ''), String(x.url || ''), String(x.content || '').slice(0, 700));
            }
        }
    }
    catch (e) {
        console.warn('Tavily fallback failed', e);
    }
    try {
        ...
        if (brave && sources.length < 5) {
            ...
            if (r.ok) {
                ...
                for (const x of (d.web?.results || []))
                    add(String(x.title || ''), String(x.url || ''), String(x.description || '').slice(0, 700));
            }
        }
    }
    catch (e) {
        ...
        if (serper && sources.length < 5) {
            ...
            if (r.ok) {
                ...
                for (const x of (d.organic || []))
                    add(String(x.title || ''), String(x.link || ''), String(x.snippet || '').slice(0, 700));
            }
        }
    }
    catch (e) {
        console.warn('Serper fallback failed', e);
    }
    return { text: blocks.slice(0, 10).join('\n'), sources: sources.slice(0, 10) };
}
function shouldSearch(message) { return /(search|google|web|internet|latest|today|current|news|headline|source|sources|खोज|सर्च|वेब|इंटरनेट|ताज़ा|आज|समाचार|खबर|न्यूज़|सोर्स)/i.test(message); }
...
async function openAiCompatibleFallback(input, system, user) {
    const providers = [
    ...
    try {
        ...
        console.warn('Legacy text fallback failed', e);
    }
...
class GeminiText {
    async generate(input) {
        ...
        let external = { text: '', sources: [] };
        if (searchRequested)
            external = await fetchWebContext(userMessage);
        const prompt = isChat ? `
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 429 || /RESOURCE_EXHAUSTED|quota/i.test(raw)) {
                    throw new Error(/PerDay|per day|daily/i.test(raw) ? 'GEMINI_DAILY_QUOTA' : 'GEMINI_RATE_LIMIT');
                }
                throw new Error(String(data?.error?.message || `Gemini API failed with status ${response.status}`));
            }
            ...
            try {
                ...
                return { titles: Array.isArray(result.titles) ? result.titles : [], description: result.description || '', tags: Array.isArray(result.tags) ? result.tags : [], hashtags: Array.isArray(result.hashtags) ? result.hashtags : [] };
            }
            catch {
            ...
            if (!isChat || input.attachmentData) {
                throw primaryError;
            }
...
class GeminiAudio {
    async generate(input) {
        ...
        if (text.length > 12000)
            throw new Error('Voice-over text is too long');
        ...
        if (!pcmBase64)
            throw new Error('Gemini TTS returned no audio. Please try again.');
...
class GeminiImage {
    async generate(input) {
        ...
        try {
            ...
            const data = await response.json();
            if (!response.ok)
                throw new Error(String(data?.error?.message || `Gemini image generation failed (${response.status})`));
            ...
            if (hedra) {
                try {
                    ...
                    const d = await r.json();
                    if (r.ok) {
                        const uri = String(d?.image_url || d?.result?.image_url || d?.output?.url || d?.result?.url || '');
                        if (uri)
                            return { imageUrl: uri, provider: 'hedra', model: process.env.HEDRA_IMAGE_MODEL || 'nano-banana-2' };
                    }
                }
                catch (e) {
                    console.warn('Hedra image fallback failed', e);
                }
            }
            try {
...
class GeminiVideo {
    async generate(input) {
        ...
        if (!first.ok) {
            ...
            if (/billing|paid|quota|not available/i.test(msg))
                throw new Error('VIDEO_PROVIDER_BILLING_REQUIRED');
        ...
        for (let i = 0; i < 90; i++) {
            ...
            const d = await r.json();
            if (!r.ok)
                throw new Error(d?.error?.message || 'Veo status check failed');
            if (d.done) {
                if (d.error)
                    throw new Error(d.error.message || 'Veo generation failed');
...
export const audioProvider = new GeminiAudio();
export const imageProvider = new GeminiImage();
export const videoProvider = new GeminiVideo();
