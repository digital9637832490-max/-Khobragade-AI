...
class GeminiText {
    async generate(input) {
        ...
        if (isChat && asksCurrentTime) {
            const timeLabel = language === 'en' ? `The current local time is ${localDateTime || 'not available'}.` : language === 'mr' ? `सध्याची स्थानिक वेळ ${localDateTime || 'उपलब्ध नाही'} आहे.` : `अभी का स्थानीय समय ${localDateTime || 'उपलब्ध नहीं है'} है.`;
            return { answer: timeLabel, webSearched: false, provider: 'context-time' };
        }
        if (isChat && asksOwnLocation) {
            if (!locationName && !(Number.isFinite(latitude) && Number.isFinite(longitude)))
                throw new Error('LOCATION_UNAVAILABLE');
            const place = locationName || `${latitude}, ${longitude}`;
            const locationLabel = language === 'en' ? `Your current location is ${place}.` : language === 'mr' ? `तुमची सध्याची लोकेशन ${place} आहे.` : `आपकी वर्तमान लोकेशन ${place} है.`;
            return { answer: locationLabel, webSearched: false, provider: 'context-location' };
        }
        const external = searchRequested ? await fetchWebContext(userMessage) : { text: '', sources: [] };
        if (isChat && searchRequested && external.sources.length === 0)
        ...
        if (gemKey) {
            try {
                ...
                if (!first.ok) {
                    ...
                    if (/billing|paid|quota|not available|permission/i.test(msg))
                ...
                if (response.ok) {
                    ...
                    if (contentType.includes('video')) {
                        ...
                        if (buf.length > 10000) {
                            ...
                            return { videoDataUrl: `data:${contentType};base64,${buf.toString('base64')}`, mimeType: contentType, provider: 'pollinations', model };
                        }
                    }
                    const text = await response.text();
                    if (/^https?:\/\//i.test(text.trim()))
                        return { videoUri: text.trim(), provider: 'pollinations', model };
                }
