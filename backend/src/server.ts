...
const app=express();
app.use(helmet());
app.use(cors({origin:[config.websiteUrl,config.adminUrl],credentials:true}));
app.use(express.json({limit:'2mb'}));
app.use(rateLimit({windowMs:config.rateWindowMs,limit:config.rateMax,standardHeaders:true,legacyHeaders:false}));
app.get('/health',(_req,res)=>res.json({ok:true,service:'creator-studio-api'}));
