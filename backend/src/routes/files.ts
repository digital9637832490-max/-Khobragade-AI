...
filesRouter.post('/files/upload', upload.single('file'), async(req,res,next)=>{
  try{
    if(!req.file) return res.status(400).json({error:'Valid file required'});
    const kind=String(req.body.kind||'project');
    const key=privateStorageKey(req.auth!.id,req.file.originalname);
    if((process.env.STORAGE_PROVIDER||'local')!=='local') return res.status(501).json({error:'Configure S3 provider implementation for non-local storage'});
    const base=path.resolve(process.cwd(),'private-storage');
    const dest=path.join(base,key);
    await fs.mkdir(path.dirname(dest),{recursive:true}); await fs.writeFile(dest,req.file.buffer);
