...
async function createAiJob(userId:string, toolKey:string, input:any){return tx(async c=>{const s=await c.query('SELECT value FROM settings WHERE key=$1',[`tool.${toolKey}`]);const cfg=s.rows[0]?.value || {enabled:true,maintenance:false};if(!cfg.enabled || cfg.maintenance) throw new Error('Tool unavailable');return (await c.query(`INSERT INTO ai_jobs(user_id,tool_key,input) VALUES($1,$2,$3) RETURNING *`,[userId,toolKey,input])).rows[0];})}
