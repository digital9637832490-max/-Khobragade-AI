...
userRouter.get('/time', async(req,res,next)=>{
  try{
    ...
    let date=new Date();
    let parts:any[]=[];
    try{parts=new Intl.DateTimeFormat('en-IN',{timeZone:tz,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).formatToParts(date);}catch{parts=new Intl.DateTimeFormat('en-IN',{timeZone:'UTC',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).formatToParts(date);}
    const get=(type:string)=>parts.find(p=>p.type===type)?.value||'';
    const validTimeZone = (()=>{ try { new Intl.DateTimeFormat('en-IN',{timeZone:tz}).format(date); return tz; } catch { return 'Asia/Kolkata'; } })();
    if(validTimeZone!==tz){ parts=new Intl.DateTimeFormat('en-IN',{timeZone:validTimeZone,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).formatToParts(date); }
...
userRouter.get('/ai/search', async(req,res,next)=>{try{const q=z.string().min(2).max(500).parse(req.query.q);const result=await searchWeb(q);if(!result.sources.length)return res.status(502).json({error:'WEB_SEARCH_UNAVAILABLE',message:'No live web search provider returned results.'});res.json(result);}catch(e){next(e)}});
...
userRouter.get('/ai/video/:id/file', async(req,res,next)=>{
  try{
    ...
    if(dataUrl.startsWith('data:video/')){
      ...
      if(comma<0)return res.status(404).json({error:'Video file is not available'});
      const mime=dataUrl.slice(5,dataUrl.indexOf(';')>0?dataUrl.indexOf(';'):comma);
      const buf=Buffer.from(dataUrl.slice(comma+1),'base64');
      res.setHeader('Content-Type',mime||'video/mp4');res.setHeader('Cache-Control','private, max-age=300');return res.end(buf);
    }
    ...
    if(apiKey && /generativelanguage\.googleapis\.com/i.test(uri)) headers['x-goog-api-key']=apiKey;
