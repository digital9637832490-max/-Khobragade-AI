...
class GeminiText implements TextProvider {
  async generate(input: Record<string, unknown>): Promise<AiResult> {
    ...
    if (isChat) {
      ...
      const hindiNums=['शून्य','एक','दो','तीन','चार','पाँच','छह','सात','आठ','नौ','दस','ग्यारह','बारह','तेरह','चौदह','पंद्रह','सोलह','सत्रह','अठारह','उन्नीस','बीस','इक्कीस','बाईस','तेईस','चौबीस','पच्चीस','छब्बीस','सत्ताईस','अट्ठाईस','उनतीस','तीस','इकतीस','बत्तीस','तैंतीस','चौंतीस','पैंतीस','छत्तीस','सैंतीस','अड़तीस','उनतालीस','चालीस','इकतालीस','बयालीस','तैंतालीस','चवालीस','पैंतालीस','छियालीस','सैंतालीस','अड़तालीस','उनचास','पचास','इक्यावन','बावन','तिरेपन','चौवन','पचपन','छप्पन','सत्तावन','अट्ठावन','उनसठ'];
      const clock=(zone:string)=>{try{const parts=new Intl.DateTimeFormat('en-IN',{timeZone:zone,hour:'numeric',minute:'2-digit',second:'2-digit',hour12:true,weekday:'long',day:'2-digit',month:'long',year:'numeric'}).formatToParts(new Date());const g=(t:string)=>parts.find(p=>p.type===t)?.value||'';const h=Number(g('hour')),m=Number(g('minute')),sec=Number(g('second'));const period=g('dayPeriod').toLowerCase();const periodHi=period.includes('pm')?'शाम':'सुबह';const safe=(n:number)=>Number.isInteger(n)&&n>=0&&n<hindiNums.length?hindiNums[n]:String(n);return {display:`${g('hour')}:${g('minute')}:${g('second')} ${g('dayPeriod')}, ${g('weekday')}, ${g('day')} ${g('month')} ${g('year')}`,speech:`${safe(h)} बजकर ${safe(m)} मिनट ${safe(sec)} सेकंड ${periodHi}`};}catch{return {display:'',speech:''}};
      ...
      if(timeQuestion){let zone=timeZone||'UTC',place='आपकी current location';for(const [city,z] of Object.entries(cityZones)){if(q.includes(city)){zone=z;place=city;break;}}const exact=clock(zone);if(exact.display)return {answer:`${place} में अभी ${exact.display} है.`,speechAnswer:`${place} में अभी ${exact.speech} है.`};if(localDateTime)return {answer:`आपके mobile के अनुसार अभी ${localDateTime} है.`};}
    }
