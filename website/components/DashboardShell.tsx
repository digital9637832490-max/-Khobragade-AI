...
const allowed=['chat','dashboard','ai-title','ai-description','ai-tags','ai-thumbnail','photo-video','voice-over','projects','notifications','support','profile'];
const labels:Record<string,string>={chat:'✨ Khobragade AI',dashboard:'Dashboard','ai-title':'AI Title','ai-description':'AI Description','ai-tags':'AI Tags','ai-thumbnail':'AI Thumbnail','photo-video':'Image & Video','voice-over':'Voice Over',projects:'Projects',notifications:'Notifications',support:'Support',profile:'Profile'};
const icons:Record<string,string>={chat:'✦',dashboard:'⌂','ai-title':'T','ai-description':'D','ai-tags':'#','ai-thumbnail':'▧','photo-video':'◉','voice-over':'♫',projects:'▣',notifications:'●',support:'?',profile:'☺'};
...
export default function Shell({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{try{setLanguage(localStorage.getItem('kh_language')||'hi')}catch{}},[]);
 const changeLanguage=(v:string)=>{setLanguage(v);try{localStorage.setItem('kh_language',v)}catch{};window.dispatchEvent(new Event('kh-language-change'));};
 ...
 return <div className="shell dashboardShell"><aside className="side proSide"><div className="sideBrand"><div style={{marginLeft:'auto',display:'flex',gap:4}}><button className="miniLang" onClick={()=>changeLanguage('en')}>EN</button><button className="miniLang" onClick={()=>changeLanguage('hi')}>हि</button><button className="miniLang" onClick={()=>changeLanguage('mr')}>म</button></div><div className="brandLogo"><img src="/khobragade-ai-logo.png" alt="Khobragade AI logo"/></div><div><b>Khobragade AI</b><small>User Panel</small></div></div><nav className="sideNav">{links.map((x,i)=><a key={x} className={path==='/' + x||path.startsWith('/'+x+'/')?'active':''} data-color={i%7} href={'/'+x}><span className="sideIcon">{icons[x]||'•'}</span><span>{labels[x]||x}</span></a>)}</nav><a className="sideLogout" href="/login" onClick={()=>localStorage.removeItem('token')}><span>↪</span> Logout</a></aside><main className="main dashboardMain">{children}</main></div>
}
